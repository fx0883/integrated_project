...
def test_tenant_admin_api():
    ...
    if login_response.status_code == 200:
        ...
        # 测试标签组API（不提供X-Tenant-ID，应自动使用用户关联的租户ID）
        print("\n===== 测试标签组API（不提供X-Tenant-ID） =====")
        tag_groups_url = "http://localhost:8000/api/v1/cms/tag-groups/"
        response = requests.get(tag_groups_url, headers=headers)
