...
const props = defineProps({
  data: {
    type: Object as () => ActiveUsersData,
...
const chartOptions = computed<EChartsOption>(() => {
  ...
  // 活跃用户数数据集
  const activeUsersDataset = props.data.datasets[0];
...
watch(
  ...
  newData => {
    if (
      newData &&
      newData.datasets &&
      newData.datasets.length > 1 &&
      options.value.series
    ) {
      const activeRateDataset = newData.datasets[1];
      const seriesConfig = {
...
watch(
  () => props.data,
  newData => {
    logger.debug(`【活跃用户统计图】数据变更 ID:${componentId}`, {
...
watch(
  ...
  () => {
    logger.debug(
      `【活跃用户统计图】周期变更，强制重新初始化图表 ID:${componentId}`
    );
    setTimeout(() => {
      forceReInit();
    }, 100);
...
watch(
  () => props.loading,
  newLoading => {
    logger.debug(`【活跃用户统计图】加载状态变更 ID:${componentId}`, {
      loading: newLoading
    });
...
<template>
  <div class="chart-container" :style="{ height }">
    ...
    <div
      ...
      class="chart"
      role="img"
      :aria-label="`${t('dashboard.activeUsers')}`"
    ></div>
  </div>
</template>
