<template>
  <div class="article-edit-page">
                ...
                >
                  <div class="version-item">
                    <div class="version-info">
                      ...
                      <span class="version-meta">{{ version.user_name }}</span>
                    </div>
    ...
    <!-- 版本查看对话框 -->
    <el-dialog v-model="versionPreviewVisible" title="版本查看" width="70%">
      <div v-loading="versionLoading" class="version-preview-container">
        <h2 class="version-preview-title">{{ versionData.title }}</h2>
        <div class="version-preview-meta">
          <span>版本时间: {{ versionData.created_at }}</span>
          <span>作者: {{ versionData.user_name }}</span>
        </div>
        <div class="version-preview-content" v-html="versionContent"></div>
      </div>
      <template #footer>
        <el-button @click="versionPreviewVisible = false">关闭</el-button>
        <el-button 
          v-if="!versionData.is_current" 
          type="primary" 
          @click="restoreVersion(versionData.id)"
        >
          恢复此版本
        </el-button>
      </template>
...
const uploadAction = computed(() => {
  return `/api/cms/articles/${articleId.value}/cover-image/`
})
...
const getArticle = async () => {
  ...
  try {
    ...
    articleForm.content_type = article.content_type || 'markdown'
    articleForm.excerpt = article.excerpt
    articleForm.status = article.status
    articleForm.published_at = article.published_at
    articleForm.cover_image = article.cover_image
    articleForm.author_name = article.author?.username || '未知作者'
    articleForm.is_featured = article.is_featured
    articleForm.is_pinned = article.is_pinned
    
    // 分类和标签
    articleForm.category_ids = article.categories?.map(c => c.id) || []
    articleForm.tag_ids = article.tags?.map(t => t.id) || []
    
    // SEO 元数据
    if (article.meta) {
      articleForm.meta_title = article.meta.title
      articleForm.meta_description = article.meta.description
      articleForm.meta_keywords = article.meta.keywords
      
      // 如果有SEO数据，默认显示SEO设置
      if (article.meta.title || article.meta.description || article.meta.keywords) {
        showSeoSettings.value = true
      }
    }
...
const loadVersions = async () => {
  ...
  try {
    const response = await articleApi.getArticleVersions(articleId.value)
...
const viewVersion = async (versionId) => {
  ...
  try {
    const response = await articleApi.getArticleVersion(articleId.value, versionId)
    versionData.value = response.data
    
    // 渲染版本内容
    if (versionData.value.content_type === 'markdown') {
      // 应该使用Markdown解析库
      versionContent.value = versionData.value.content
    } else {
      versionContent.value = versionData.value.content
    }
  } catch (error) {
...
const restoreVersion = async (versionId) => {
  try {
    await ElMessageBox.confirm('确定要恢复到该版本吗？当前未保存的修改将丢失。', '警告', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
    
    await articleApi.restoreArticleVersion(articleId.value, versionId)
